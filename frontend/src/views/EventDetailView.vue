    ...
    <section v-else-if="detail" class="space-y-6">
      ...
      <header class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            type="button"
            class="flex shrink-0 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"
            @click="copyShareUrl"
          >
            分享
          </button>
        </div>
        <p v-if="shareMessage" class="mt-2 text-sm text-emerald-600">{{ shareMessage }}</p>
