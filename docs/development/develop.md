...
| **MVP-2.1** | 訂單與 hold 機制 | ✅ 完成 |
| **MVP-2.2** | 金流（ECPay） | ✅ 完成 |
| **MVP-2.3** | 訂單狀態機 + 出票 | ✅ 完成 |
| **MVP-2.4** | 庫存安全與背景任務（Phase 3） | ✅ 完成 |
| **MVP-2.5** | 報名表單擴充（Phase 4.1） | ✅ 完成 |
| **MVP-2.6** | 基礎退款（Phase 4.2） | ✅ 完成 |
...
- **[.claude/skills/ecpay](.claude/skills/ecpay)**：綠界官方 Cursor Agent Skill，用於 ECPay 金流整合（AIO、CheckMacValue、Webhook 驗簽等）。
...
1. **金流相關**：若涉及付款、Webhook、ECPay/Stripe，**必先閱讀** [.claude/skills/ecpay](.claude/skills/ecpay)
